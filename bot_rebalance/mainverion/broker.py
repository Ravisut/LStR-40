...
class broker :
    ...
    def call_bbands(self,timeframe,Period):
        if self.what_broker == 'FTX':
            ...
            return data
    def call_atr(self,timeframe,Period):
        if self.what_broker == 'FTX':
            data = self.ftxObject.atr(timeframe,Period)
            return data
...
class FTX() :
    ...
    def atr(self, timeframe,Period):

        ohlcatr_df = self.OHLC(timeframe,Period)
        ...
        ohlcatr_df['ATR'] = ohlcatr_df['TR'].ewm(span = Period).mean()
        ohlcatr_df['SMA'] = ohlcatr_df['close'].rolling(Period).mean()
        ##################

        return  ohlcatr_df['ATR'].iloc[-1]
